...
// Auto-switch: advance effect every AUTO_SWITCH_INTERVAL_MS milliseconds
#define AUTO_SWITCH_INTERVAL_MS 30000u
static volatile bool     g_auto_switch_enabled = true;
static volatile uint32_t g_last_switch_ms      = 0;
...
typedef enum { IMU_TYPE_NONE = 0, IMU_TYPE_BMI220, IMU_TYPE_BMI270 } imu_type_e;
static imu_type_e   g_imu_type   = IMU_TYPE_NONE;
...
static bool imu_read(float *acc_x, float *acc_y, float *gyr_x, float *gyr_y);
...
static void buzzer_button_ok_cb(char *name, TDL_BUTTON_TOUCH_EVENT_E event, void *argc)
{
    if (event == TDL_BUTTON_PRESS_SINGLE_CLICK || event == TDL_BUTTON_PRESS_DOUBLE_CLICK) {
        ...
        g_last_switch_ms = tal_system_get_millisecond(); // reset auto-switch timer
        PR_NOTICE("OK Button: animation mode -> %d", g_animation_mode);
    } else if (event == TDL_BUTTON_LONG_PRESS_START) {
        g_auto_switch_enabled = !g_auto_switch_enabled;
        PR_NOTICE("OK Button long press: auto-switch %s", g_auto_switch_enabled ? "ON" : "OFF");
    }
}

/**
 * @brief Button A callback - previous effect
 */
static void buzzer_button_a_cb(char *name, TDL_BUTTON_TOUCH_EVENT_E event, void *argc)
{
    if (event == TDL_BUTTON_PRESS_SINGLE_CLICK || event == TDL_BUTTON_PRESS_DOUBLE_CLICK) {
        uint32_t total_animations = EFFECT_ANIMATION_COUNT + g_registered_pixel_art_count;
        g_animation_mode = (g_animation_mode + total_animations - 1) % total_animations;
        g_last_switch_ms = tal_system_get_millisecond();
        PR_NOTICE("A Button: animation mode -> %d", g_animation_mode);
    }
}

/**
 * @brief Button B callback - buzzer / sand physics
 */
static void buzzer_button_b_cb(char *name, TDL_BUTTON_TOUCH_EVENT_E event, void *argc)
{
    if (event == TDL_BUTTON_PRESS_SINGLE_CLICK) {
        ...
        if (OPRT_OK != rt) {
            PR_ERR("Failed to play melody: %d", rt);
        }
        ...
        g_auto_switch_enabled = false; // sand mode stays until manually changed
        g_animation_mode = SAND_PHYSICS_MODE;
        PR_NOTICE("B Button long press: sand physics mode");
    }
}
...
/**
 * @brief Read accelerometer + gyroscope from whichever IMU is active.
 *        Returns true if data was successfully read.
 */
static bool imu_read(float *acc_x, float *acc_y, float *gyr_x, float *gyr_y)
{
    if (g_imu_type == IMU_TYPE_BMI220 && g_bmi220_dev != NULL &&
        board_bmi220_is_ready(g_bmi220_dev)) {
        bmi220_sensor_data_t raw = {0};
        if (board_bmi220_read_data(g_bmi220_dev, &raw) == OPRT_OK) {
            *acc_x = raw.acc_x;
            *acc_y = raw.acc_y;
            *gyr_x = raw.gyr_x;
            *gyr_y = raw.gyr_y;
    ...
    } else if (g_imu_type == IMU_TYPE_BMI270 && g_bmi270_dev != NULL &&
               board_bmi270_is_ready(g_bmi270_dev)) {
        bmi270_sensor_data_t raw = {0};
        if (board_bmi270_read_data(g_bmi270_dev, &raw) == OPRT_OK) {
            *acc_x = raw.acc_x;
            *acc_y = raw.acc_y;
            *gyr_x = raw.gyr_x;
            *gyr_y = raw.gyr_y;
...
static void sand_update_physics(void)
{
    ...
    imu_read(&acc_x, &acc_y, &gyr_x, &gyr_y);
...
static void pixel_led_animation_task(void *args)
{
    ...
    while (g_animation_running) {
        /* Auto-switch every 30 s (skip sand physics mode) */
        if (g_auto_switch_enabled && g_animation_mode != SAND_PHYSICS_MODE) {
            uint32_t now_ms = tal_system_get_millisecond();
            if ((now_ms - g_last_switch_ms) >= AUTO_SWITCH_INTERVAL_MS) {
                uint32_t total = EFFECT_ANIMATION_COUNT + g_registered_pixel_art_count;
                g_animation_mode = (g_animation_mode + 1) % total;
                g_last_switch_ms = now_ms;
                PR_NOTICE("Auto-switch: animation mode -> %d", g_animation_mode);
            }
        }
...
static void user_main(void)
{
    ...
    // Initialize IMU: try BMI220 first, fall back to BMI270
    ...
    if (g_bmi220_dev != NULL) {
        ...
        if (!board_bmi220_is_ready(g_bmi220_dev)) {
            ...
            if (OPRT_OK == rt) {
                ...
                PR_NOTICE("BMI220 initialized successfully");
            } else {
            ...
            g_imu_type = IMU_TYPE_BMI220;
            PR_NOTICE("BMI220 already initialized");
        }
    }
        ...
        if (g_bmi270_dev != NULL) {
            ...
            if (!board_bmi270_is_ready(g_bmi270_dev)) {
                ...
                if (OPRT_OK == rt) {
                    ...
                    PR_NOTICE("BMI270 initialized successfully");
                } else {
                ...
                g_imu_type = IMU_TYPE_BMI270;
                PR_NOTICE("BMI270 already initialized");
            }
        }
    }
    if (g_imu_type == IMU_TYPE_NONE) {
        PR_WARN("No IMU available — sand physics will use base gravity only");
    }
    ...
    PR_NOTICE("  OK  single/double: next effect  |  long: toggle auto-switch (30s)");
    PR_NOTICE("  A   single/double: prev effect");
    PR_NOTICE("  B   single: play melody  |  long: sand physics (auto-switch OFF)");
    PR_NOTICE("==========================================");
