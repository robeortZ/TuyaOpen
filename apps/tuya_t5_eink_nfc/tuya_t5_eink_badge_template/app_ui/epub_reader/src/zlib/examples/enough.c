...
{
    return ((size_t)((syms - 1) >> 1) * ((syms - 2) >> 1) + (left >> 1) - 1) * (g.max - 1) + len - 1;
}
...
local big_t count(int syms, int left, int len)
{
    ...
    int most = (((code_t)left << (g.max - len)) - syms) / (((code_t)1 << (g.max - len)) - 1);
...
local void examine(int syms, int left, int len, int mem, int rem)
{
    ...
    if (syms == left) {
        ...
        if (mem >= g.large) {
            ...
            string_printf(&g.out, "<%u, %u, %u>:", syms, g.root + 1, ((1 << g.root) - left) << 1);
    ...
    for (use = least; use <= most; use++) {
        g.code[len] = use;
        examine(syms - use, (left - use) << 1, len + 1, mem + (rem ? 1 << (len - g.root) : 0), rem << 1);
        if (rem == 0) {
            rem = 1 << (len - g.root);
            mem += rem;
        }
