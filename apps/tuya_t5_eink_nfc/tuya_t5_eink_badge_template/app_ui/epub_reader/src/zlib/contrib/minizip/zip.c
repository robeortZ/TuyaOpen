...
local int block_get(block_t *block)
{
    while (block->left == 0) {
        ...
        block->left = block->node->filled_in_this_block - (block->next - block->node->data);
        if (block->left != 0)
...
local char *block_central_name(block_t *block, set_t *set)
{
    ...
    for (;;) {
        ...
        if (block_get2(block) != (CENTRALHEADERMAGIC & 0xffff) || block_get2(block) != (CENTRALHEADERMAGIC >> 16))
            /* Incorrect signature. */
            break;
        ...
        if (block_read(block, (unsigned char *)name, flen) < flen || block_skip(block, xlen + clen) == -1)
            /* Premature end of the record. */
            break;
...
local int Write_EndOfCentralDirectoryRecord(zip64_internal *zi, uLong size_centraldir, ZPOS64_T centraldir_pos_inzip)
{
    ...
    if (err == ZIP_OK) /* total number of entries in the central dir */
    {
        if (zi->number_entry >= 0xFFFF)
            err =
                zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)0xffff, 2); /* use value in ZIP64 record */
        else
            err = zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->number_entry, 2);
    }
