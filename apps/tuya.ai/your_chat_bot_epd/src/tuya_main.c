...
// static void epd_app_thread(void *arg)
// {
//     int ret = OPRT_OK;
//     extern OPERATE_RET EPD_7in5_V2_init(void);
//     ret = EPD_7in5_V2_init();
//     if (ret != OPRT_OK) {
//         PR_ERR("EPD_7in5_V2_test failed");
//         tal_thread_delete(epd_app_thread_handle);
//         epd_app_thread_handle = NULL;
//     }
// }
static TIMER_ID epd_time_update_tm;
...
#if defined(ENABLE_CHAT_DISPLAY) && (ENABLE_CHAT_DISPLAY == 1)
        app_display_send_msg(TY_DISPLAY_TP_STATUS, (uint8_t *)time_str, strlen(time_str));
#endif
    }
...
void user_main(void)
{
    ...
    ret = EPD_7in5_V2_init();
    if (ret != OPRT_OK) {
        PR_ERR("EPD_7in5_V2_init failed");
    }
    ...
    ret = tal_sw_timer_create(__app_display_status_time_update, NULL, &epd_time_update_tm);
    if(ret != OPRT_OK) {
        PR_ERR("create epd_time_update_tm failed, ret:%d", ret);
    }else{
        PR_DEBUG("create epd_time_update_tm success");
        tal_sw_timer_start(epd_time_update_tm, 10*1000 , TAL_TIMER_CYCLE);
    }
    for (;;) {
