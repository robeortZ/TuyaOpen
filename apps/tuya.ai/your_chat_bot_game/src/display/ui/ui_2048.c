...
// 定义全局变量存储初始角度
static lv_display_rotation_t g_init_rotation = LV_DISPLAY_ROTATION_0;
...
static void on_back_to_chat(lv_event_t *e)
{
    if (g_2048.prev_screen) {
        ...
        lv_disp_set_rotation(disp, g_init_rotation);  // 恢复原始旋转
...
{
    lv_disp_t * disp = lv_disp_get_default();
    g_init_rotation = lv_disp_get_rotation(disp);
    lv_disp_set_rotation(disp, LV_DISP_ROTATION_180);  // 旋转0度
    ui2048_build_screen();
